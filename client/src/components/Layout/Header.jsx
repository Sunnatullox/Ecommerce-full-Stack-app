...
const Header = ({ activeHeading }) => {
  ...
  return (
    <>
      <div className={`${styles.section}`}>
        <div className="hidden 800px:h-[50px] 800px:my-[20px] 800px:flex items-center justify-between">
          ...
          <div className={`${styles.button}`}>
            <a href={`${isSeller ? "/dashboard" : "/shop-create"}`}>
      ...
      <div
        className={`${window.scrollY > 75 ? "" : "hidden "}w-full h-[50px]`}
      ></div>
      <div
        className={`${
              ...
              <div className="relative cursor-pointer mr-[15px]">
                {isAuthenticated ? (
                  <Link to="/profile" className="relative">
                    {messages.length > 0 ? (
                      <span className="absolute top-[-10px] left-5 inline-flex px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">
                        {messages.length}
                      </span>
                    ) : null}
                    <img
